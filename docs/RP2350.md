...
- In some of my use-cases the RP2350 can be clocked to the same as an STM32 and achieve the same results.  In some cases, it needs to be clocked faster to achieve the same results.  I suspect this is due to differences in the flash and memory subsystems.
...
- My first PCB revision **just worked** - meaning the first PCB layout was a success.  (The firmware didn't quite work first time after porting, but only took a few hours to get fully functional.)
...
I might be able to reduce contention by using either PIOs to serve bytes (it has direct GPIO connection), or using the Coprocessor port 0 (ditto).  I will probably try.

I did upload the ROM image to be served to a dedicated SRAM bank to try and reduce contention.  Maybe that helped?

What is the Coprocessor port 0?  It appears to be a direct connection between the Cortex core and the GPIO interface, hence avoiding the bus matrix.  It involves uses some ARMv8-M instructions to access.  See 3.6.1 GPIO Coprocessor (GPIOC) from the RP2350's datasheet.
